...
class SBaseOptions():
    ...
    def initialize(self, parser):
        ...
        parser.add_argument('--dataset_mode', type=str, default='sn', help='aligned,single')
        parser.add_argument('--model', type=str, default='sn', help='chooses which model to use. bicycle,, ...')
        ...
        parser.add_argument('--netD3', type=str, default='basic_256_multi', help='selects model to use for netD2')
        ...
        parser.add_argument('--zoom_nums', type=int, default=2, help='how many levels of zoom net')
        parser.add_argument('--randomrotation', action='store_true', help='if true, random rotate images')
        parser.add_argument('--colorJitter', action='store_true', help='if true, use ColorJitter')

        parser.add_argument('--seams_map', action='store_true', help='if true, add map data to seams dataset')

        parser.add_argument('--conD', action='store_true', help='conditional discriminator loss with diamond mask.')

        parser.add_argument('--more_samples', action='store_true', help='add more data to discriminator')

        parser.add_argument('--forced_mask', action='store_true', help='if true, force generated mask edges to be white')
        parser.add_argument('--fade', action='store_true', help='if true, fade 5 pixels in forced mask')
        parser.add_argument('--g1_mask', action='store_true', help='if true, add a faded circle to G1')
    ...
    def gather_options(self):
        ...
        model_option_setter = smodels.get_option_setter(model_name)
        parser = model_option_setter(parser, self.isTrain)
        opt, _ = parser.parse_known_args()  # parse again with new defaults

        # modify dataset-related parser options
        dataset_name = opt.dataset_mode
        dataset_option_setter = sdata.get_option_setter(dataset_name)
