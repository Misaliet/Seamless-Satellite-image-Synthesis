...
def save_images(webpage, images, names, image_path, aspect_ratio=1.0, width=256):
    ...
    for label, im_data in zip(names, images):
        ...
        if aspect_ratio < 1.0:
            ...
            im = im.resize((int(h / aspect_ratio), w), Image.BICUBIC)
        # print("--------------")
        # print(im.shape)
        # print("--------------")
        # if im.shape[2] > 3 and nn == 0:
            # print("--------------")
            # im = im[:, :, : 3]
            # im = im[:, :, 0: 3]
        # if im.shape[2] > 3:
                ...
                try:
                    ...
                    for i in range(0, len(images)):
                        ...
                        if images[i].shape[0] > 3:
                            ...
                            # images[i] = images[i][3: 6, 0: 256, 0: 256]
                            # Fix the error when input and output images are not the same size
                        if images[i].shape[1] < 256:
                            t = np.zeros((3,256,256))
                            # t[: , 96: 160, 96: 160] = images[i]
                            t[: , 64: 192, 64: 192] = images[i]
                            images[i] = t
        ...
        if self.use_html and (save_result or not self.saved):  # save images to an HTML file if they haven't been saved.
            ...
            for label, image in visuals.items():
                ...
                if image_numpy.shape[2] > 3:
                    ...
                    # image_numpy = image_numpy[0: 256, 0: 256, 3: 6]
                # print("------------")
