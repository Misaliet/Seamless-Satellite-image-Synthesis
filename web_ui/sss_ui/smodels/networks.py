...
def define_G(input_nc, output_nc, nz, ngf, netG='unet_128', norm='batch', nl='relu',
    ...
    if netG == 'unet_128' and where_add == 'input':
        ...
        net = G_hr(input_nc, output_nc, nz, 8, ngf, norm_layer=norm_layer, nl_layer=nl_layer,
                             use_dropout=use_dropout, upsample=upsample)
    else:
...
def define_E(input_nc, output_nc, ndf, netE,
    ...
    if netE == 'resnet_128':
        ...
        net = E_ResNet(input_nc, output_nc, ndf, n_blocks=5, norm_layer=norm_layer,
                       nl_layer=nl_layer, vaeLike=vaeLike)
        # net = E_ResNet(input_nc, output_nc, ndf, n_blocks=8, norm_layer=norm_layer,
                    #    nl_layer=nl_layer, vaeLike=vaeLike)
    elif netE == 'conv_128':
...
def define_Z(input_nc, output_nc, nz, ngf, netG='unet_128', norm='batch', nl='relu',
             use_dropout=False, init_type='xavier', init_gain=0.02, gpu_ids=[], where_add='input', upsample='bilinear', nums=3):
    net = None
    norm_layer = get_norm_layer(norm_type=norm)
    nl_layer = get_non_linearity(layer_type=nl)
    ...
    if netG == 'hr':
        net = G_hr(input_nc, output_nc, nz, 8, ngf, norm_layer=norm_layer, nl_layer=nl_layer,
                             use_dropout=use_dropout, upsample=upsample, nums=nums)
    else:
        raise NotImplementedError('Generator model name [%s] is not recognized' % net)
...
class D_NLayers(nn.Module):
    ...
    def __init__(self, input_nc, ndf=64, n_layers=3, norm_layer=nn.BatchNorm2d, nl_layer=nn.LeakyReLU):
...
class G_Unet_add_input(nn.Module):
    def __init__(self, input_nc, output_nc, nz, num_downs, ngf=64,
        ...
        # unet_block = UnetBlock_crop(input_nc + nz, output_nc, ngf, unet_block,
                            #    outermost=True, norm_layer=norm_layer, nl_layer=nl_layer, upsample=upsample)
...
class UnetBlock_crop(nn.Module):
    def __init__(self, input_nc, outer_nc, inner_nc,
                 submodule=None, outermost=False, innermost=False,
                 norm_layer=None, nl_layer=None, use_dropout=False, upsample='basic', padding_type='zero'):
                 # original padding_type='zero'
        super(UnetBlock_crop, self).__init__()
        self.outermost = outermost
        # print("11111111111111")
        p = 0
        downconv = []
